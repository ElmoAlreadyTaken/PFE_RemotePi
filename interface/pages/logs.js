...
export default function AdminPage() {
  ...
  const [serverIP, setServerIP] = useState("376b-2a01-cb01-63-d761-7794-44a0-47c6-3136.ngrok-free.app"); // Nouvelle adresse IP du serveur
  ...
  return (
    ...
    <button onClick={clearLogs} className="mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
      Effacer
    </button>
    {errorMessage && (
    ...
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg" style={{ maxHeight: '400px', overflowY: 'auto' }}>
  <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
  <tr>
    <th scope="col" style={{ width: '10%' }} className="px-6 py-3">Index</th>
    <th scope="col" style={{ width: '90%' }} className="px-6 py-3">Message/Error</th>
  </tr>
</thead>
    ...
    <tbody>
      {logList.map((log, index) => (
        <tr key={index}>
          ...
          <td className={`px-6 py-4 ${log.error ? 'text-red-500' : 'text-black'}`}>
